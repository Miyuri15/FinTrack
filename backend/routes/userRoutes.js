...
router.post("/register", async (req, res) => {
    const { firstName, lastName, email, contactNumber, password, confirmPassword, role, adminName, organization } = req.body;
    ...
    try {
        ...
        if (role === "admin") {
            if (!adminName || !organization) {
                console.log("Validation failed: Admin name and organization are required"); // Debugging
                return res.status(400).json({ message: "Admin name and organization are required" });
            }
        }
